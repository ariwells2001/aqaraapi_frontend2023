...
def all(location,username,password,rows,tableRows):
    ...
    full_list = []
    DN = rows
    st.subheader("Information on Devices")
    ...
    print("FUll List is also {}".format(full_list))
    # logoff = st.sidebar.button("Logoff")
    # st.write("Devices List",full_list)
    placeholder1 = st.sidebar.empty()
    full_list.insert(0,"All")
    with placeholder1.form("Detail"):
        page = st.selectbox("Choose a device",full_list)
        testButton = st.form_submit_button("Detail")
    
    if testButton:

         if page == "All":
             devices(location,username,password,rows,tableRows)
         elif page == "weather":
             dummyFunc()
...
if __name__ == '__main__':
    all(location,username,password)
